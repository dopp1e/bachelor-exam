...
Określa, jak rośnie liczba operacji wraz ze wzrostem rozmiaru danych wejściowych.
Opiera się na operacjach, którą uznajemy za podstawowe (np. dodanie dwóch liczb, porównanie dwóch liczb, przypisanie wartości do zmiennej).

=== Złożoność pamięciowa

Jest to miara określająca ilość pamięci wykorzystywanej przez algorytm.
Określa, jak rośnie ilość zużywanej pamięci przez algorytm wraz ze wzrostem rozmiaru danych wejściowych.
...
Wiedza kubaliczna:
- $tilde(O)$ (soft O, O z tyldą) - ignoruje logarytmy w określaniu złożoności.
  Np. $tilde(O)(n)$ oznacza $O(n log^k n)$ dla pewnego stałego k.
- $tilde(Theta)$ (soft Theta, Theta z tyldą) - ignoruje logarytmy w określaniu złożoności.
  Np. $tilde(Theta)(n)$ oznacza $Theta(n log^k n)$ dla pewnego stałego k.
...
#table(
  ...
  table.header([*Kategoria*], [*Rodzaj*], [*Przykłady*], [*Uwagi*]),
...
#table(
  ...
  table.header([*Złożoność Czasowa*], [*Opis*], [*Przykład*]),
...
#table(
  ...
  table.header([*Algorytm*], [*Złożoność czasowa*], [*Złożoność pamięciowa*]),
...
==== Przykłady złożoności pamięciowej
...
+ W jakich jednostkach podaje się złożoność asymptotyczną np. $O(n)$? (Boiński)
...
+ Czy algorytm o mniejszej złożoności czasowej zawsze wykona się szybciej? (Daciuk)
+ Jakie mamy trzy złożoności algorytmów? (Janczewski)
...
+ Podać kilka algorytmów z różnymi zależnościami pamięciowymi i określić jakie mają. (autor nieznany)
  - Wyszukiwanie binarne w posortowanej tablicy - $O(1)$
  - Sortowanie przez scalanie (merge sort) - $O(n)$
  - Rekursywna implementacja Fibonacciego - $O(n)$
  - Iteracyjna implementacja Fibonacciego - $O(1)$
+ Podaj złożoności quicksorta. (Boiński)
  - Średnia złożoność czasowa: $O(n log n)$
  - Najgorsza złożoność czasowa: $O(n^2)$
  - Złożoność pamięciowa: $O(log n)$
+ W jakich jednostkach podaje się złożoność asymptotyczną np. $O(n)$? (Boiński)
  - Liczba operacji podstawowych w zależności od wielkości danych wejściowych $n$.
+ Przykłady algorytmów w danej złożoności. (Dereniowski)
  - $O(1)$ - Dostęp do elementu tablicy
  - $O(log n)$ - Wyszukiwanie binarne
  - $O(n)$ - Przeszukiwanie tablicy
  - $O(n log n)$ - Sortowanie szybkie (quicksort)
  - $O(n^2)$ - Sortowanie bąbelkowe
+ W jakiej jednostce jest mierzona złożoność czasowa? (Szłapczyńska)
  - W liczbie operacji podstawowych wykonywanych przez algorytm w zależności od wielkości danych wejściowych.
+ W jakiej strukturze danych szukanie jest $O(1)$? (Szwoch)
  - W tablicy haszującej (hash table, hash map).
+ Czy algorytm o mniejszej złożoności czasowej zawsze wykona się szybciej? (Daciuk)
  - Nie, ponieważ złożoność czasowa to tylko asymptotyczna miara wydajności. Rzeczywisty czas wykonania zależy również od stałych czynników, takich jak koszty operacji podstawowych, optymalizacje kompilatora, oraz rozmiar danych wejściowych. Kubale kiedyś wspominał o takiej kategorii algorytmów jak algorytmu galaktyczne. Algorytmy te mają bardzo dobrą złożoność asymptotyczną, ale ze względu na duże stałe czynniki są wolniejsze dla praktycznych rozmiarów danych wejściowych, czasem niewykonywalne nawet dla małych danych wejściowych
+ Jakie mamy trzy typy złożoności algorytmów? (Janczewski)
  - Optymistyczną (najlepszy przypadek), pesymistyczną (najgorszy przypadek), oczekiwaną (średni przypadek).
+ Podaj przykład algorytmu o stałej złożoności pamięciowej. (Manus)
  - Iteracyjna implementacja obliczania n-tej liczby Fibonacciego.
+ Czy i dlaczego ignorujemy stałe w określaniu złożoności obliczeniowej? (Waloszek)
  - Ignorujemy stałe, ponieważ złożoność obliczeniowa skupia się na asymptotycznym zachowaniu algorytmu dla rozmiaru danych wejściowych dążącego do nieskończoności. Stałe nie mają istotnego wpływu na skalowalność.
