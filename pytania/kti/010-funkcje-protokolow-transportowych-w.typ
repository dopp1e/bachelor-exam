...
Do zrywania połączenia używa mechanizmów:
- bit RST w segmencie TCP do natychmiastowego zakończenia połączenia,
- timeouty, które automatycznie zamykają połączenie po określonym czasie bezczynności.
...
Mechanizmy zarządzania prędkością transmiji tak by dopasować ją do możliwości obydwu stron:
...
=== Protokół QUIC
*QUIC (Quick UDP Internet Connections)* to protokół transportowy opracowany przez Google, który działa na bazie UDP i jest zaprojektowany do szybkiego i bezpiecznego 
przesyłania danych w internecie. QUIC integruje funkcje takie jak szyfrowanie, kontrola przepływu i wielościeżkowość, co pozwala na redukcję opóźnień i poprawę wydajności w porównaniu do tradycyjnych protokołów transportowych, takich jak TCP.
